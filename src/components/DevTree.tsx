...
export default function DevTree({ data }: DevTreeProps) {
  ...
  const handleDragEnd = (e: DragEndEvent) => {
    ...
    if (over && over.id) {
      ...
      ); // keep disabled links unchanged

      const links = [...order, ...disabledLinks]; // combine enabled and disabled links
      console.log(links);
      ...
      <div className="min-h-screen py-10 bg-gray-100">
        <main className="max-w-5xl p-10 mx-auto md:p-0">
          ...
          <div className="flex justify-end">
            <Link
              className="text-2xl font-bold text-right scale-105 text-slate-800 hover:cursor-pointer hover:text-green-400 hover:bg-slate-600 hover:px-2 hover:rounded-lg"
              to={`/${data.handle}`}
              target="_blank"
